...
# issues:
    ## Danenbergh2022 - this runs an R script to convert file types (manual in windows)
    ### complete! 
...
#dataset.intensity.to_csv("Danenberg2022.csv")
# analysis
# --- Corr ---
...
# PCA plot - try PC2XPC3?
axs[0].scatter(pca_result[:, 1], pca_result[:, 2], 
               facecolors='none',
               edgecolors='gray',
               s=10,
               alpha=0.7)
axs[0].set_xlabel(f'PC2 [A.U.] ({pca.explained_variance_ratio_[1]:.1%} variance)')
axs[0].set_ylabel(f'PC3 [A.U.] ({pca.explained_variance_ratio_[2]:.1%} variance)')
...
axs[1].scatter(tsne_result[:, 0], tsne_result[:, 1],     
               ...
               s=10,
               alpha=0.7)
axs[1].set_xlabel('t-SNE 1 [A.U.]')
axs[1].set_ylabel('t-SNE 2 [A.U.]')
...
axs[2].scatter(umap_result[:, 0], umap_result[:, 1], 
               ...
               s=10,
               alpha=0.7)
axs[2].set_xlabel('UMAP 1 [A.U.]')
axs[2].set_ylabel('UMAP 2 [A.U.]')
